  ...
  if status:
    ...
    #since CoreML only allows loading constant of rank-3: [C,H,W])
      ...
      if c == 1: #swap seq. and C
        ...
        context.builder.add_load_constant(name, output_name + '_pre_permute', 
            x, [seq, h, w]) 
...
def _add_concat(op, context):
  ...
  if axis == 3: #concatenate along channel axis
    ...
    for i, input_name in enumerate(input_names):
      context.builder.add_permute(output_name + '_' + str(i), (0,3,2,1), 
          input_name, input_name + '_swap_W_C_' + str(i))
      inputs_permuted.append(input_name + '_swap_W_C_' + str(i))
    context.builder.add_elementwise(output_name + '_concat', inputs_permuted, 
        output_name + '_concat', 'CONCAT')
    context.builder.add_permute(output_name, (0,3,2,1), 
        output_name + '_concat', output_name)
  elif axis == 1: #concatentae along heigth axis        
    inputs_permuted = []
    for i, input_name in enumerate(input_names):
      context.builder.add_permute(output_name + '_' + str(i), (0,2,1,3), 
...
def _add_mean(op, context):
  ...
  if status:
    ...
    if len(input_shape) == 4 and (np.array_equal(axis_ind, np.array([0,1,2])) \
