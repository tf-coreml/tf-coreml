...
def _expand_dims(op, blob_name, output_name, context):
    ...
    assert len(input_shape) == len(output_shape)-1, 'Expand Dimension must increase the rank by 1'
...
def _reshape(op, blob_name, output_name, context):
  ...
  #handle the very specific case when input is [X] and output is [X,1]    
...
#Will make the interpret_shape function return False, making shape interpretation fall back to static mapping    
...
def _interpret_shape(blob_name, context):
    ...
    dim_labels: Tensor name to labeled shapes (one of 'S','C','H','W'). e.g.: 'input' tensor which has shape (1,224,224,3) --> ('S','H','W','C')
    """
    ...
    if blob_name in context.dim_labels:
        ...
        if len(ops_list) == 0:
            ...
            if not status:
                ...
                # The shape of "output_name" of "op" has been interpretted. Now we are asking to interpret the shape of the input to this op: "blob_name"
